...
describe('AuthPanel', () => {
  ...
  it('rejects a stale persisted user session during startup revalidation', async () => {
    sessionStorage.setItem('agent_kb_auth_session_v1', JSON.stringify({
      token: 'stale-session', kind: 'session', username: 'admin1', role: 'admin',
      expiresAt: '2099-01-01T00:00:00.000Z',
    }));
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
      if (String(input) === '/api/auth/status') {
        return { ok: true, status: 200, json: async () => ({ mode: 'multi_user', userCount: 1 }) } as Response;
      }
      return {
        ok: false, status: 401, json: async () => ({ error: 'A valid session token is required. Log in again.' }),
      } as Response;
    }));

    render(<AuthPanel />);

    expect(await screen.findByRole('alert')).toHaveTextContent('valid session token');
    expect(screen.queryByText('Signed in as admin1')).not.toBeInTheDocument();
